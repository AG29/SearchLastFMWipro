...
// Defines types of hash string outputs available
 ...
 extension String {
    ...
    public func replace(_ word : String , By : String, CaseSensitive : Bool = true ) -> String
    {
        if CaseSensitive {
           return self.replacingOccurrences(of: word, with: By)
        }
        
        let start = self.index(self.startIndex, offsetBy: 0)
        let end = self.index(self.startIndex, offsetBy: self.count)
        let range = start..<end
        let value = self.replacingOccurrences(of: word, with: By, options: .caseInsensitive, range: range)
        return value
    }
